...
class GraphExtractor:
    def __init__(self, config: Configuration, delimiter: str = '|', dump_file: bool = False):
        self.cf = config
        self.logging = self.cf.logging
    ...
    def build_encoder(self):
        ...
        try:
            assert len(aspect_key_ids) == len(aspect_val_ids)
        except AssertionError:
            self.logging.debug('key-value mapping size not equal: {} vs {}'.format(len(aspect_key_ids), len(aspect_val_ids)))
    ...
    def make_pairs(self, aspects: list):
        ...
        for i in range(len(aspects)):
            for j in range(i+1, len(aspects)):
                if func(aspects[i]) in self.aspect_id_mapping and func(aspects[j]) in self.aspect_id_mapping:
...
if __name__ == '__main__':
    ...
    kg_extractor = GraphExtractor(config=config, dump_file=True)
